...
export const RESOLVED_ADAPTER_BROWSER_STUB_ID = '\0ripple-new:adapter-browser-stub';
...
export function serve() {
  throw new Error('[ripple-new] Server adapters cannot run in the browser.');
}
export function nodeRequestToWebRequest() {
  throw new Error('[ripple-new] Node request helpers cannot run in the browser.');
}
export function webResponseToNodeResponse() {
  throw new Error('[ripple-new] Node response helpers cannot run in the browser.');
}
`;
}
...
/**
 * Generate the client hydration entry (served at virtual:ripple-new-hydrate).
 *
...
export function create_client_entry_source(_options) {
	return `// Auto-generated by @ripple-ts/vite-plugin-new.
...
(async () => {
  try {
    ...
    if (!el || !target) {
      console.error('[ripple-new] Unable to hydrate: missing #__ripple_data or #root.');
      return;
    }
    const data = JSON.parse(el.textContent || '{}'); // { entry, exportName, layout, params }
    if (!data.entry) {
      console.error('[ripple-new] Unable to hydrate: no route entry in #__ripple_data.');
      return;
    }
    ...
    if (!Component) {
      console.error('[ripple-new] Unable to hydrate: no component export for', data.entry);
      return;
    }
    ...
    if (data.layout) {
      ...
      if (Layout) {
        // children is a ComponentBody closing over params; ripple-new's childSlot
    ...
    console.error('[ripple-new] Failed to bootstrap client hydration.', error);
  }
})();
`;
}
