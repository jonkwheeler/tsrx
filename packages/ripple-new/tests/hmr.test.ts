 ...
 * the ripple-new compiler emits. They follow the standard
...
describe('hmr — runtime wrapper', () => {
	...
	it('preserves hook state across update() — stable Symbol.for keys', () => {
		...
		// `Symbol.for('ripple-new:file.tsrx:Foo.useState#0')`-stable emit).
		const HOOK_SYM = Symbol.for('ripple-new:hmr.test.ts:Counter.useState#0');
		const v1: ComponentBody<any> = ((scope: Scope, _props: any, _extra: any) => {
	...
	it('compiler emits Symbol.for(...) for hook slots and an import.meta.hot.accept block', async () => {
		const { compile } = await import('@tsrx/ripple-new');
		const src =
		...
		expect(code).toMatch(/Symbol\.for\("ripple-new:file\.tsrx:Foo\.useState#0"\)/);
