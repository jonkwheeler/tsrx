...
function styleSheetTextFor(el: Element): string {
	...
	const sheets = Array.from(
		document.head.querySelectorAll('style[data-ripple-new]'),
	) as HTMLStyleElement[];
...
function getInjectedStyles(): string[] {
	return Array.from(document.querySelectorAll('style[data-ripple-new]')).map(
		(s) => s.getAttribute('data-ripple-new')!,
	);
}
