...
// ripple-side useEffect body reads its deps positionally (an ripple-new-
...
// RefInIf belongs to the ripple-new-only conformance suite — it pins
...
describe('differential: useref.tsrx — DOM refs (callback form)', () => {
	it('DomRefCallback: callback ref fires with element on mount', async () => {
		...
		// and ripple-new. Per the brief: don't pre-emptively make it pass — let
...
// than the ripple-new positional-deps spread shape, so both runtimes drive
// the same writes into the shared log. ripple-new still supports the
