...
A registry of intentional behavior differences between ripple-new's Suspense
...
**ripple-new behavior:** We eagerly commit each sibling as its individual promise
...
**ripple-new behavior:** The `@catch (err, reset)` positional `reset` is an
ripple-new-specific TSRX syntax — same intent (retry the failed branch with fresh
...
past it). The test pins ripple-new's specific per-replay call-count behavior so a
