...
/**
 * ripple-new-preview — Start the production SSR server.
 *
...
try {
	...
	if (!fs.existsSync(entryPath)) {
		console.error(`[ripple-new-preview] Server entry not found: ${entryPath}`);
		console.error('[ripple-new-preview] Did you run `pnpm build` first?');
		process.exit(1);
	}

	console.log(`[ripple-new-preview] Starting server from ${outDir}/server/${ENTRY_FILENAME}`);
	...
	console.error('[ripple-new-preview] Failed to load config:', error.message);
	process.exit(1);
}
